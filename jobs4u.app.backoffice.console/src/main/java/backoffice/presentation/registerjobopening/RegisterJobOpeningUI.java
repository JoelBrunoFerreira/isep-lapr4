...
public class RegisterJobOpeningUI extends AbstractUI {
    ...
    protected boolean doShow() {
        try {
            Iterable<CustomerDTO> customers = customerController.getCustomerList();
            SelectWidget<CustomerDTO> customerSelector = new SelectWidget<>("Select the customer corresponding to the job opening:", customers, new CustomerPrinter());
            ...
            if (customer != null) {
                ...
                SelectWidget<String> modeSelector = new SelectWidget<>("Mode:", controller.getModeList());
                modeSelector.show();
                final String mode = modeSelector.selectedElement();

                SelectWidget<String> contractTypeSelector = new SelectWidget<>("Contract type:", controller.getContractTypeList());
                contractTypeSelector.show();
                final String contractType = contractTypeSelector.selectedElement();

                JobOpeningDTO resultDTO = controller.registerJobOpening(title, description,
                        address, numberVacancies, mode, contractType, customer.getId());
